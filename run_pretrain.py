...
parser.add_argument('-a', '--arch', metavar='ARCH', default='vit', choices=model_names,
                    help='model architecture: ' + ' | '.join(model_names + ['vit']) + ' (default: vit)')
parser.add_argument('--vit-path', '-vp', type=str, help='path of the pretrained vit model',
                    choices=['none', 'first', 'second'])
parser.add_argument('--image_res', '-ir', type=int, help='resolution of images', default=128)
...
def main():
    ...
    # query_name_list 6215
    # binary_train_data 142355
    # train_dataset 136139
    train_dataset = dataset.get_dataset(args.dataset_name, args.image_res, args.n_views)
    ...
    if args.arch != 'vit':
        ...
        model = ViTSimCLR(model_path=args.vit_path, image_res=args.image_res)
